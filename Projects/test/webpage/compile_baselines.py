...
def get_baseline_video(path):
    title = ["Diffusion as Shader", "Go-With-The-Flow", "ATI", "Tora"]
    model_names = ["dasFirst_0", "gwtf_0", "ati_49_40", "tora_0_40"]
    lis = path.split("/")
    model_name = lis[-2]
    video_name = lis[-1]
    b_path = []
    ...
    for i in range(len(model_names)):
        path = os.path.join("/".join(lis[:-3]), model_names[i], model_names[i], video_name)
        if os.path.exists(path):
            b_path.append(path)
            b_title.append(title[i])

    path = os.path.join("/efs/users/yuancheng.xu/public/Koichi_track/DATASETS/pexel_custom2_stylize/tora", video_name)
    if os.path.exists(path):
        b_path.append(path)
        b_title.append("Tora")
    
    return b_path, b_title
...
def get_vis_video(path):
    ...
    tmp_folder = "./../tmp_webpage/tmp_vis/" + ((path.split("/")[-1]).split(".mp4")[0]) + "_camera"
    os.makedirs(tmp_folder, exist_ok = True)
...
for video_name in dic:
    ...
    inputs["Track condition"] = vis_path

    inputs["TrackR2V (Ours)"] = examples[0]
    
    #Tile baselines
    b_path, b_title = get_baseline_video(examples[0])
    
    for b_idx in range(len(b_path)):
        #Stack together (GT, motion tracks, stylized videos)
        inputs[b_title[b_idx]] = b_path[b_idx]
    
    #Stack videos
    output_video_path = os.path.join("./baselines", video_name + ".mp4")
